...
let arrayRandomCitiesHtml = '';

for (let i = 0; i < 22; i++) {
  const randomIndex = Math.floor(Math.random() * listOfCities.length);
  arrayRandomCitiesHtml += `<a href="/Mudanzas/${listOfCities[randomIndex].replaceAll(' ', '-')}">${listOfCities[randomIndex]}</a> `;
 
}

let alterateLinks = document.getElementById('alterateLinks');
if(alterateLinks){
    alterateLinks.innerHTML = arrayRandomCitiesHtml;
}
